...
- `Listener`: процесс слушает SRT на `0.0.0.0:<port>` либо на адресе выбранного выходного интерфейса; в `gst-launch-1.0` режим дополнительно задаётся явным `mode=listener`. Поле `Адрес выхода` используется только как адрес, показываемый в ссылке клиенту, и не используется как bind-адрес.
...
### Ошибки запуска потока в web-интерфейсе

Release 2 возвращает реальный результат `/api/start-stream` в web-интерфейс. Если поток не удалось запустить (например, отсутствует GStreamer-плагин, SRT Caller не имеет адреса сервера, SRT Listener не смог привязать порт или внешний `gst-launch-1.0` завершился при старте), интерфейс показывает закреплённое красное окно **«Ошибка запуска потока»**.

Окно не исчезает по таймеру и остаётся на экране, пока пользователь не нажмёт на него или на кнопку `×`. Это позволяет прочитать длинное сообщение GStreamer и не потерять диагностическую информацию при следующем обновлении `/api/state`.

Для внешнего GStreamer-транскодера stderr `gst-launch-1.0` перехватывается в течение стартового окна. Если процесс завершается сразу, текст GStreamer добавляется к ответу API и показывается в окне ошибки. Для SRT старт контролируется дольше, чтобы успеть поймать ошибки bind/URI.

SRT Listener после транскодирования использует выбранный выходной интерфейс как реальный bind-адрес и явно задаёт одновременно `mode=listener`, `localaddress` и `localport`. Пример ожидаемой команды:

```text
srtsink uri=srt://192.168.148.200:5101?mode=listener \
  mode=listener localaddress=192.168.148.200 localport=5101 \
  latency=250 wait-for-connection=false
```

При проблеме запуска сначала прочитайте сообщение в интерфейсе, затем при необходимости проверьте журнал:

```bash
journalctl -u tvstreamer5 -n 150 --no-pager | grep -Ei 'srt|gst-launch|GStreamer transcoder|error|failed|bind|exit'
```
