  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full" id="streamWisiCompatibilityRow"><label>Совместимость WISI</label><div class="checkbox-inline"><input id="streamWisiCompatibility" type="checkbox" ${stream.wisi_compatibility ? 'checked' : ''} /><span>WISI CHAMELEON</span></div><small>Только для UDP-CBR без транскодирования. Сервис пересобирается в чистый SPTS без CBR-padding внутри mpegtsmux; отдельный WISI TS-shaper использует временные метки PTS/DTS выходных буферов mpegtsmux, добавляет NULL PID 0x1FFF до Target bitrate и переписывает PCR по фактическому выходному clock. Обычный UDP-CBR остаётся без изменений.</small></div>
