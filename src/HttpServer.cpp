  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full"><label>Target bitrate (кбит/с, TS CBR)</label><input id="streamBitrate" type="number" value="${Math.round((stream.target_bitrate||2000000)/1000)}" placeholder="2000" /><small>Для транскодированных MPEG-TS выходов UDP-CBR, HTTP, HLS, SRT и RTP задаёт транспортный CBR. Для HLS сетевой HTTP-трафик остаётся сегментным.</small></div>
