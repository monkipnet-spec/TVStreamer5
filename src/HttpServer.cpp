  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full" id="streamWisiCompatibilityRow"><label>Совместимость WISI</label><div class="checkbox-inline"><input id="streamWisiCompatibility" type="checkbox" ${stream.wisi_compatibility ? 'checked' : ''} /><span>WISI CHAMELEON</span></div><small>Только для UDP-CBR без транскодирования. WISI shaper создаёт реальный стартовый резерв 2 секунды, адаптивно поддерживает запас по PTS/DTS, добавляет NULL PID 0x1FFF до Target bitrate и переписывает PCR по фактическому выходному clock. Обычный UDP-CBR остаётся без изменений.</small></div>
