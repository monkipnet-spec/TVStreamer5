  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full" id="streamWisiCompatibilityRow"><label>Совместимость WISI</label><div class="checkbox-inline"><input id="streamWisiCompatibility" type="checkbox" ${stream.wisi_compatibility ? 'checked' : ''} /><span>WISI CHAMELEON</span></div><small>Только для UDP-CBR без транскодирования. Сервис пересобирается в чистый SPTS CBR с битрейтом из Target bitrate; после mux поток равномерно выдаётся byte-clock pacer по GStreamer clock. Обычный UDP-CBR остаётся без изменений.</small></div>
